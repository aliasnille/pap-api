...
**Note** Only the parameters `s` (street address) and `c` (city) can be combined. To address validation separate street, number, postal code and city with `|` (vertical bar). Values may not contain any spaces, these must be replaced with `+` (plus).
